...
type CompE struct {
	...
	permBlue  []int
	permGreen []int
	comp2Ref  *CompB // Reference to Comp2
	comp3Ref  *CompC // Reference to Comp3
}

// Function to calculate the permutation of an array
func (c *CompE) calculatePerm(arr []int) []int {
...
func (c *CompE) sumArrays(arr1, arr2 []int) []int {
...
func (c *CompE) mergeAndRemoveDuplicates(slice1, slice2 []bg.Component) []bg.Component {
...
func (c *CompE) ProcessReq(request bg.Request[interface{}]) {
	time.Sleep(1 * time.Second)
	depCompRefs1 := c.comp2Ref.getReferences()
	depCompRefs2 := c.comp3Ref.getReferences()
	...
	fmt.Println("Process completed in CompE")
}
...
func (c *CompE) Sync() {
	...
	fmt.Println("Sync completed in CompE")

}

func (c *CompE) Cancel() {
	fmt.Println("Component E cancelling")
	stagingVersion := c.GetStagingVersion()
	switch stagingVersion {
	case 0:
		c.arrBlue = c.arrGreen
...
func (c *CompE) GetLiveData(index int) int {
...
func (c *CompE) GetStagingData() interface{} {
	stagingVersion := c.GetStagingVersion()
	switch stagingVersion {
	case 0:
		return c.arrBlue
...
func (c *CompE) getReferences() []bg.Component {
	var depCompRef []bg.Component
	depCompRef = append(depCompRef, c.comp2Ref.getReferences()...)
	depCompRef = append(depCompRef, c.comp3Ref.getReferences()...)
...
func (c *CompE) GetStagingDatas() []int {
	stagingVersion := c.GetStagingVersion()
	switch stagingVersion {
	case 0:
		return c.permBlue
